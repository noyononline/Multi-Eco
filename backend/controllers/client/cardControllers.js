...
class cardControllers {
  add_to_card = async (req, res) => {
    ...
    try {
      const existingProduct = await cardModel.findOne({ userId, productId });
      if (existingProduct) {
  ...
  add_to_wishlist = async (req, res) => {
    ...
    try {
      const prod = await wishlistModel.findOne({ slug });
      if (prod) {
        responseReturn(res, 404, {
          error: "Already added product",
        });
      } else {
